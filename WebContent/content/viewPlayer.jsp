...
<div class="scheda analitica">
  ...
  <s:if test="careerList.size() > 0">
    <s:iterator value="careerList" status="careerStatus">
      ...
      <tr class="bg2">
        <td style="text-align: left; margin: 0px"><s:if test="monthTransfer neq null && monthTransfer neq ''">(<s:property value="monthTransfer" />)</s:if></td>
        <td>
       		<s:if test="team.nation.id == 2 || team.nation.id == 54">${fn:substring(stagione.value, 0,4)}</s:if>
       		<s:if test="team.nation.id != 2 && team.nation.id != 54"><s:property value="stagione.value" /></s:if>        		
        </td>        
        <td style="text-align: left;"><s:property value="serie" /></td>
